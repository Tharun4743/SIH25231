...
export default function AudioRecorder({ onTranscriptionComplete, onError }: AudioRecorderProps) {
  ...
  const handleTranscription = async (blob: Blob) => {
    ...
    try {
      ...
      // If backend returned simulated flag, Whisper is not installed
      if (result.simulated === true) {
        onError(
          "Whisper not installed. Run: pip install faster-whisper — then restart the backend to enable real voice transcription."
        );
        return;
      }
